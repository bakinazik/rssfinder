...
function getSiteKey(url) {
    try {
        ...
        return `${urlObj.protocol}//${urlObj.host.replace(/^www\./, '')}`;
    } catch (e) {
        return url;
    }
}
...
async function verifyFeed(url) {
    ...
    try {
        ...
        const sample = text.slice(0, 10000).trim();

        return (
            /^<rss(?:\s|>)/i.test(sample) ||
            /^<feed(?:\s|>)/i.test(sample) ||
            /^<rdf:RDF(?:\s|>)/i.test(sample) ||
            /^<\?xml[\s\S]*?<rss(?:\s|>)/i.test(sample) ||
            /^<\?xml[\s\S]*?<feed(?:\s|>)/i.test(sample) ||
            /^<\?xml[\s\S]*?<rdf:RDF(?:\s|>)/i.test(sample)
        );
    } catch (error) {
        return false;
    } finally {
...
async function collectPotentialRSSFeeds(currentUrl, guessTitle) {
    const feeds = [];
    ...
    function addFeed(url, title, source = 'html') {
        ...
        try {
            ...
            if (foundUrls.has(normalizedUrl)) return;
    ...
    commonPaths.forEach(path => {
        try {
            addFeed(new URL(path, urlObj.origin).href, guessTitle, 'guess');
        } catch (e) {}
    });
    ...
    if (urlObj.hostname.includes('youtube.com')) {
        ...
        for (let i = 0; i < 6; i++) {
            metaChannelId =
                ...
                document.querySelector('meta[itemprop="identifier"]')?.getAttribute('content');

            if (metaChannelId) break;
        ...
        if (metaChannelId) {
            addFeed(
                `https://www.youtube.com/feeds/videos.xml?channel_id=${metaChannelId}`,
                'YouTube',
                'platform'
            );
        }
        ...
        const match =
            currentUrl.match(/youtube\.com\/channel\/([^\/\?&]+)/) ||
        ...
        if (match) {
            ...
            if (currentUrl.includes('/@')) {
                if (!metaChannelId) {
                    ...
                    if (handleChannelId) {
                        addFeed(
                            `https://www.youtube.com/feeds/videos.xml?channel_id=${handleChannelId}`,
                            'YouTube',
                            'platform'
                        );
                    }
    ...
    if (urlObj.hostname.includes('reddit.com')) {
        ...
        if (match) {
            addFeed(
                `https://www.reddit.com/r/${match[1]}.rss`,
                'Reddit',
                'platform'
            );
        }
...
async function startScanning(tabId, url) {
    ...
    try {
        ...
        if (
            prevData?.siteKey === siteKey &&
            prevData?.feeds?.length > 0
        ) {
            setBadge(
                tabId,
                prevData.feeds.length
            );

            tabData.verifiedFeeds = [...prevData.feeds];

            tabData.discoveredUrls = new Set(
                prevData.feeds.map(f => f.url)
            );
        } else {
            setBadge(tabId, 0);
        }
    ...
    try {
        const results = await chrome.scripting.executeScript({
            ...
            args: [
                url,
                chrome.i18n.getMessage('feedGuessTitle')
            ]
        });
        ...
        const linkFeeds = potentialFeeds.filter(
            f => f.source === 'link'
        );

        const guessFeeds = potentialFeeds.filter(
            f => f.source !== 'link'
        );

        for (const feed of linkFeeds) {
            if (
                !await tabExists(tabId) ||
                !tabData.isScanning ||
                tabData.url !== url
            ) {
                return;
            ...
            if (tabData.discoveredUrls.has(feed.url)) {
        ...
        const verifyPromises = guessFeeds.map(async feed => {
            ...
            if (
                isValid &&
                !tabData.discoveredUrls.has(feed.url)
            ) {
                feed.verified = true;
                feed.verifiedAt = Date.now();
        ...
        if (scanningTabs.get(tabId) === tabData) {
            scanningTabs.delete(tabId);
        }
    }
}
...
chrome.tabs.onActivated.addListener(activeInfo => {
    chrome.tabs.get(activeInfo.tabId, tab => {
        if (
            ...
            !isScannableUrl(tab?.url)
        ) {
            return;
        }

        chrome.storage.session.get(
            [`rssFeeds_${activeInfo.tabId}`],
            result => {
                const data =
                    result[`rssFeeds_${activeInfo.tabId}`];
...
chrome.runtime.onMessage.addListener(
    (request, sender, sendResponse) => {
        if (request.action === 'getScanStatus') {
            chrome.storage.session.get(
                [`rssFeeds_${request.tabId}`],
                async result => {
                    const data =
                        result[`rssFeeds_${request.tabId}`];

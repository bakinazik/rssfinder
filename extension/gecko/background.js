...
async function restoreFromStorage(tabId, siteKey, url) {
    try {
        const stored = await storageGet([`rssFeeds_${tabId}`]);
...
function setBadge(tabId, count) {
    try {
        if (count > 0) {
            ...
            browser.browserAction.setBadgeBackgroundColor({ color: '#faa501', tabId });
        } else {
...
browser.tabs.onRemoved.addListener((tabId) => {
    scanningTabs.delete(tabId);
    storageRemove(`rssFeeds_${tabId}`);
});

browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'getScanStatus') {
        storageGet([`rssFeeds_${request.tabId}`]).then(async (result) => {
            const data = result[`rssFeeds_${request.tabId}`];
            try {
                const tab = await browser.tabs.get(request.tabId);
                if (!tab?.url || !isScannableUrl(tab.url)) {
                    sendResponse({ isScanning: false, feeds: [], isValid: false });
                    return;
                }
                const currentSiteKey = getSiteKey(tab.url);
    ...
    if (request.action === 'refreshScan') {
        browser.tabs.get(request.tabId).then((tab) => {
            if (!tab?.url || !isScannableUrl(tab.url)) {
                sendResponse({ started: false, error: 'Tab not found' });
                return;
            }
